...
for iteration in range(num_iterations):
    ...
    for ant, length in zip(tours, tour_lengths):
        for i in range(n - 1):
            # Update pheromones for the directed edge from ant[i] to ant[i+1]
            pheromones[ant[i]][ant[i + 1]] += pheromone_deposit / length
        # Update pheromones for the edge from the last city back to the starting city
        pheromones[ant[-1]][ant[0]] += pheromone_deposit / length

# Plot the best tour with arrows (smaller arrows)
...
for i in range(len(best_tour) - 1):
    plt.arrow(
        tour_lons[i], tour_lats[i],
        tour_lons[i + 1] - tour_lons[i], tour_lats[i + 1] - tour_lats[i],
        head_width=0.003, head_length=0.003, fc='blue', ec='blue', linestyle='-', linewidth=1, alpha=0.6
    )
...
plt.ylabel('Latitude')
plt.legend()
plt.title('Best Tour for TSP with Smaller Directed Arrows')
plt.grid(True)
plt.show()
