...
# Filter customers and restaurants based on the same date
date = '25-01-2024'  # Replace this with the desired date
same_date_customers = customers_df[customers_df['date_x'] == date][['lat1', 'long1']].apply(tuple, axis=1).tolist()
same_date_restaurants = restaurants_df[restaurants_df['date_y'] == date]

# Nearest neighbor algorithm
customer_counter = 1
restaurant_counter = 1
while same_date_customers:
    min_distance = float('inf')
    nearest_restaurant = None
    
    # Find nearest restaurant from current location
    for idx, restaurant in same_date_restaurants.iterrows():
    ...
    for customer_coords in same_date_customers:
    ...
    same_date_customers.remove(nearest_customer[1])
...
print("Optimized route for date", date + ":")
