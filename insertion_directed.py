...
# Visualize the traversal path as arrows
plt.figure(figsize=(10, 6))
plt.scatter(longitude, latitude, c='red', marker='o', label='Cities')
for i in range(n):
    plt.arrow(
        longitude[tour[i]], latitude[tour[i]], 
        longitude[tour[i + 1]] - longitude[tour[i]], latitude[tour[i + 1]] - latitude[tour[i]], 
...
plt.ylabel('Latitude')
plt.legend()
plt.title('Traversal Path for TSP with Starting Point (Arrows)')
plt.grid(True)
plt.show()
